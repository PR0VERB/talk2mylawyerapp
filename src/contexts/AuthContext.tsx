...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (session?.user) {
        // In a real app, you'd fetch the user role from your profiles table
        setUserRole('client'); // Default for now
      }
      setLoading(false);
    });
  ...
  const signUp = async (email: string, password: string, role: 'client' | 'lawyer') => {
    ...
    // In a real app, you'd also create a profile record with the role
  };
